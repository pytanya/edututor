...
2. answer_type="single": ровно 4 варианта в options (1 верный + 3 правдоподобных
   дистрактора того же типа), без дублей и одинаковых по смыслу. _correct_answer обязан
   ТОЧНО совпадать с текстом одного из options.
3. answer_type="open": options = [], _correct_answer — эталонный краткий ответ.
