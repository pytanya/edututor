...
| Инструмент | Источники | Назначение |
|------------|-----------|-------------|
| `rag_search` | векторная база (in-memory) | факты из учебного материала; при пустой базе возвращает заметку «База пуста → web_search» вместо пустого массива |
| `web_search` | Yandex (RU) / Tavily (Global) → DDG fallback | актуальная интернет-информация |
| `generate_quiz` | отдельный LLM-вызов | короткая карточка квиза `{question, options, answer_type, _correct_answer}` для типа quiz |
...
1. Проверить, не отключён ли инструмент circuit breaker'ом инструмента
   (`ToolFailureTracker`).
2. Выполнить вызов под таймаутом `TOOL_TIMEOUT_SEC` (15 c); инструменты,
   внутри которых выполняется отдельный LLM-вызов (`generate_quiz`), получают
   полноценный `TUTOR_LLM_TIMEOUT_SEC`.
...
### Почему такие модели (регион RU)

Выбор основной (planner) модели зафиксирован по результатам живых прогонов на
шлюзе RouterAI (см. также `LLMClientFactory.get_models_for_region` в `src/llm/base.py`):

| Роль | Модель | Обоснование |
|------|--------|-------------|
| `planner` (RU) | `qwen/qwen3.7-flash` | Основная роль в референсе `project_work`; instruct-модель без reasoning — стабильно отдаёт JSON-конверты, нативные `tool_calls` и корректный LaTeX `$...$`/`$$...$$`. |
| `planner` fallback | `deepseek/deepseek-chat`, `qwen/qwen-2.5-7b-instruct` | Instruct-кандидаты другого вендора на случай недоступности/пустого ответа qwen3.7-flash. |
| `fast` | `qwen/qwen-2.5-7b-instruct` | Лёгкая модель для коротких вызовов. |
| `judge` | `google/gemini-2.5-flash` | Асессор качества ответа (Critic). |

Почему НЕ `deepseek/deepseek-v4-flash-0731`: это **reasoning-модель** — перед
ответом тратит токены на «размышления» (поле `reasoning`). При тесном
`max_tokens` она съедает весь бюджет на reasoning и возвращает **пустой**
`content` с `finish_reason="length"` (подтверждено 4/4 пустыми прогонами на
промпте агента). Именно поэтому от reasoning-моделей отказались ещё на раннем
этапе (исторически — из-за пустых ответов `deepseek-r1`). v4-flash-0731 можно
включить через `TUTOR_LLM_PLANNER_MODEL`, если шлюз разгрузится, но тогда нужен
запас по `TUTOR_LLM_MAX_TOKENS` (дефолт 6144).

Общий принцип: **instruct-модель без скрытого reasoning** — детерминированность
формата (JSON-конверт) важнее «думающей» модели; формат конверта — строгий
контракт с парсером `src/agent/envelope.py`.
