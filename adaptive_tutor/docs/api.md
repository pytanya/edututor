...
### Адаптивный советник сложности заданий (LinUCB)

Опционально (включён по умолчанию; отключается `TUTOR_BANDIT_ENABLED=false`)
перед каждым ходом сервер может добавить в контекст агента **system-совет** с
рекомендуемой сложностью следующего задания: LinUCB contextual bandit с руками
`easy`/`medium`/`hard` подстраивается под контекст ученика и темы. Если ход
выпускает задание (`quiz`/`practice`), выбранная рука фиксируется на сессии
(`bandit_arm`/`bandit_topic`/`bandit_features`) и после хода-ответа
(`evaluation`) используется для обновления бандита (награда — корректность
ответа), после чего контекст руки очищается. Совет — **мягкая рекомендация**:
`difficulty` ответа (и `adaptive.difficulty`) выбирает модель, значение не
нормализуется. Изменений во фронтенде не требуется.
